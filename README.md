...
- **`apps/collect_workspace.py`** -- PyQt6 메인 GUI (워크스페이스형). VS Code 식 레이아웃: 왼쪽 액티비티 바가 왼쪽 패널만 바꾸고 — 순서는 수집 한 바퀴 그대로다 (① Layout 카메라 점검 → ② Configure Scene 설정 → ③ Collect 수집 → ④ Dataset 큐레이션, 그 뒤로 Statistics/Upload/Settings; Ctrl+1~7), **카메라 미리보기는 항상 중앙에 유지된다** -- 3단계 마법사(준비/수집/정리)를 대체한 것으로, 단계 전환마다 화면 전체가(카메라까지) 바뀌던 문제를 없앴다. 중앙은 Live/Playback 탭, 오른쪽은 로봇·카메라·기록 상태, 아래는 Log/Upload/Validation 탭, 맨 아래 상태바(노드·카메라 생사 + 저장 경로 여유 + fps + 경로). 로봇 노드 시작/종료, 자세 매칭 게이트(조인트별 델타 바), 에피소드 제어, 데이터셋 탐색기와 20fps 재생, 재압축·HDF5 업로드·LeRobot 변환/업로드까지 한 창에서 처리한다. 모든 패널은 스플리터로 자유롭게 조절된다.
