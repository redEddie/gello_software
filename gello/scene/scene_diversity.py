...
함정 3) 거리 기반은 scene→task 상관을 보장하지 못한다.
  ...
  (:mod:`gello.scene.skill_stats` — 실행 가능한 스킬 중 누적 수집횟수가 적은
  것 우선). 상관 자체는 이 모듈이 아니라 감사 도구
  (``scripts/analyze/audit_scene_diversity.py``)가 별도 지표로 감시한다.
"""
...
def recommend_detailed(existing: list, props: dict, k: int = 3,
    ...
    지시문 결정은 여기서 하지 않는다 — :mod:`gello.scene.skill_stats` 로 별도
    단계에서 (함정 3).
    """
