...
def build_collect(win) -> QWidget:
    ...
    win.slot_plan_combo.currentIndexChanged.connect(win._on_slot_plan_pick)
    sfrm.addRow(tr("계획 slot"), win.slot_plan_combo)
    win.slot_next_btn = QPushButton(tr("다음 미수집 slot 제시"))
    win.slot_next_btn.clicked.connect(win._on_next_slot)
    ...
    win.slot_instr_edit.editingFinished.connect(win._on_slot_sentence_edited)
    sfrm.addRow(tr("문장"), win.slot_instr_edit)
    win.slot_apply_btn = QPushButton(tr("slot 적용 (다음 에피소드부터)"))
    win.slot_apply_btn.clicked.connect(win._on_apply_slot)
