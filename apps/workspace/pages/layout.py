...
def build_layout_page(win) -> QWidget:
    ...
    win.layout_suite_combo.currentIndexChanged.connect(win.scene_ops.layout_refilter)
    sform.addRow(tr("Suite"), win.layout_suite_combo)
    nav = QWidget()
    nrow = QHBoxLayout(nav)
    nrow.setContentsMargins(0, 0, 0, 0)
    prev_btn = QPushButton("◀")
    prev_btn.clicked.connect(lambda: win.scene_ops.layout_step(-1))
    nrow.addWidget(prev_btn)
    win.layout_play_btn = QPushButton(tr("일시정지"))
    win.layout_play_btn.clicked.connect(win.scene_ops.layout_toggle_play)
    nrow.addWidget(win.layout_play_btn, 1)
    next_btn = QPushButton("▶")
    next_btn.clicked.connect(lambda: win.scene_ops.layout_step(+1))
    ...
    win.layout_interval_combo.currentIndexChanged.connect(
        win.scene_ops.layout_apply_interval)
    ...
    win.layout_alpha_slider.valueChanged.connect(win.scene_ops.layout_alpha_changed)
    ...
    win.layout_blink_check.toggled.connect(win.scene_ops.layout_blink_toggled)
    ...
    win.layout_blink_slider.valueChanged.connect(
        win.scene_ops.layout_blink_interval_changed)
    ...
    win.layout_grid_check.toggled.connect(
        lambda _on: win.scene_ops.layout_rerender())
    ...
    ws_grid_btn.clicked.connect(win.scene_ops.on_edit_grid)
