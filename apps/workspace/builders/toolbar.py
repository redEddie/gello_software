...
def _build_menu(win) -> None:
    ...
    m.addSeparator()
    m.addAction(tr("용량 최적화 (재압축)..."), win._on_repack)
    m.addAction(tr("LeRobot 변환/업로드..."), win._on_lerobot)
    m.addSeparator()
    m.addAction(tr("전체 처리 (재압축 → 변환 → 업로드)..."), win._on_pipeline)
    m.addAction(tr("HDF5 업로드..."), win._on_hdf5_upload)
    ...
    m.addAction(tr("Hugging Face 계정..."), win._on_hf_accounts)
    m.addSeparator()
