"""Scene configuration, slot planning, and layout-delegation operations."""
...
class SceneOps:
    """Scene configuration, plan management, slot handling, and layout delegation."""
    ...
    def on_start_sentence_edited(self) -> None:
        self.auto_assign_iid(self.win.lang_edit.text(), self.win.scene_iid_edit,
    ...
    def refresh_start_plan_combo(self) -> None:
        ...
        sid = self.configure_scene_id()
        if plan is not None and sid is not None:
            counts: dict = {}
            if self.win.session.scene_session and sid == self.session_scene_id():
                # 세션이 파일을 쥐고 있다 -- saver 가 보내준 캐시로 센다
                counts = self.session_slot_counts()
            else:
                p = self.selected_scene_path()
    ...
    def on_slot_sentence_edited(self) -> None:
        ...
        self.auto_assign_iid(self.win.slot_instr_edit.text(), self.win.slot_iid_edit,
                              scene_id=self.session_scene_id(),
    ...
    def refresh_slot_panel(self) -> None:
        ...
        sid = self.session_scene_id() if self.win.session.scene_session else None
        counts = self.session_slot_counts()
    ...
    def layout_refilter(self, *_args):
        self.win._layout_refilter()

    def layout_step(self, delta, user=True):
        self.win._layout_step(delta, user=user)

    def layout_toggle_play(self):
        self.win._layout_toggle_play()

    def layout_apply_interval(self, *_args):
        self.win._layout_apply_interval()

    def layout_alpha_changed(self, val):
        self.win._layout_alpha_changed(val)

    def layout_blink_toggled(self, on):
        self.win._layout_blink_toggled(on)

    def layout_blink_interval_changed(self, ms):
        self.win._layout_blink_interval_changed(ms)

    def layout_rerender(self):
        self.win._layout_rerender()

    def layout_blink_tick(self):
        self.win._layout_blink_tick()
